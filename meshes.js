    ...
    for (let x = 0; x < 10; ++x) {
        for (let z = 0; z < 10; z++) {
            if (x == 0 && z == 0) {
                ...
                //data.meshWithMaterial.push(data.mesh[data.mesh.length - 1]);
                data.rgbNipple.push(data.mesh[data.mesh.length - 1]);
            } else {
                data.mesh.push(data.mesh[meshIndex].clone());
                //data.mesh.push(data.mesh[meshIndex].createInstance());
            }
...
function createBlockChamfLinesBack(){
       ...
       let nippleLineShape0 = [new BABYLON.Vector3(-0.21, 1.35, -0.29),
        ...
        scene.meshes.pop();
        // lineSystem = BABYLON.MeshBuilder.CreateLineSystem(null, {lines: line, instance: lineSystem});
    //lineSystem = BABYLON.MeshBuilder.CreateLineSystem(null, { lines: _lines, instance: lineSystem });
   // lineSystem.position.z = 1.5;
    //lineSystem.position.y = 1.5;
...
/*
  ...
  //  let blockArray = [blockBody, blockChamfNipple1, blockChamfNipple2];
    //let blockChamfArray = [blockChamfBody, blockChamfNipple1, blockChamfNipple2];
